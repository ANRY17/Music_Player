...
if (isset($_POST['submit'])) {
    ...
    if (array_filter($errors)) {
        ...
        //IF GET ID -> UPDATE IT
        if (isset($_GET['id'])) {
            $updateSinger = "UPDATE singers SET name = '" . mysqli_real_escape_string($conn, $singername) . "', info = '" . mysqli_real_escape_string($conn, $info) . "', image = '$images' WHERE id = $id";
            $res3 = mysqli_query($conn, $updateSinger);
            if (!$res3) {
                echo  "Error: " . mysqli_error($conn);
                exit; // Exit the script in case of error
            } else {
                header("Location: editSinger.php");
                exit; // Exit the script after redirecting
            }
        } else {
            $insertSinger = "INSERT INTO singers (name, info, image) VALUES ('" . mysqli_real_escape_string($conn, $singername) . "', '" . mysqli_real_escape_string($conn, $info) . "', '$images')";
